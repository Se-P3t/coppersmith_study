...
def sage_small_roots(poly, X=None, beta=1.0, epsilon=None, **kwds):
    r"""
    Let `N` be the characteristic of the base ring this polynomial
    is defined over: ``N = poly.base_ring().characteristic()``.
    ...
    ZmodN = poly.base_ring()
    roots = set([ZmodN(r) for r,m in R if abs(r) <= X])
    Nbeta = N**beta
    return [root for root in roots if N.gcd(ZZ(poly(root))) >= Nbeta]


if __name__ == '__main__':
    raise NotImplementedError("you should directly use `sage.rings.polynomial.polynomial_modn_dense_ntl.small_roots`")
